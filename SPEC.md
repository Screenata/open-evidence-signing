...
|---|---|---|
| `@context` | MUST | Always `"https://openevidence.dev/signing/v1"` |
...
|---|---|---|
| 1.0 | 2026-09-02 | **Namespace migration**: canonical `@context` is now `https://openevidence.dev/signing/v1` (the previously named `openevidence.org` domain was never controlled by the spec authors). Not a version bump: `@context` sits outside the signed `subject`, so no signature is invalidated — an envelope issued under the old URI verifies after updating its `@context` field to the canonical value. The `.org` URI is no longer valid and is rejected on verify. |
